...
def draw_squares_from_circle(n, circle, window):
    ...
    for k in range(n):
        square = rg.Square(rg.Point((cx + k * radius), (cy + k * radius)), radius * 2)
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    rectanglew = rectangle.get_lower_right_corner().x - rectangle.get_lower_left_corner().x
    rectangleh = rectangle.get_upper_right_corner().y - rectangle.get_lower_right_corner().y
    radius1 = rectangleh / 2
    radius2 = rectanglew / 2

    rectangle1 = rectangle
    rectangle1.attach_to(window1)

    for k in range(m):
        circle = rg.Circle(rg.Point((radius1 + rectangle.get_upper_left_corner().x + (2*k * radius1)), (rectangle.get_lower_left_corner().y + radius1)), radius1)
